...
class MyNotebookPage():
    ...
    def addCauiEyeScanControls(self,frame,device,device_list,debug=False): 
      if(debug):
        print 'Add CAUI Controls'
...
class MyFrame(wx.Frame):
    def __init__(self, parent, id, title, system):
        ...
        self.SetTitle("Inphi System Controller - Version 1.09") # FIXME! - need global var        
        ...
        for eachDevice in ipvar.System.keys():
            if (re.search(r'\w+-\w+',eachDevice)):
              ...
              if(cdr) :
                #print 'CDR: %s'%cdr.group(1)
                self.CDR_list.append(cdr.group(1)) 
              if(gb1) :
                #print 'GB1: %s'%gb1.group(1)
                self.GB1_list.append(gb1.group(1))
              if(gb2) :
                #print 'GB2: %s'%gb2.group(1)
                self.GB2_list.append(gb2.group(1))               
        
        print 'ipallreg - CDRs: %s' % self.CDR_list
        print 'ipallreg - GB1s: %s' % self.GB1_list
        print 'ipallreg - GB2s: %s' % self.GB2_list
                
        self.Device_list = self.CDR_list + self.GB1_list + self.GB2_list
        ...
        #----- GB1 Menu Items ------------------------------------------------- 
        for eachDevice in self.GB1_list:
        ...
        for eachDevice in self.GB2_list:
          if '_' in eachDevice:
            ...
            if eachDevice.find('GB2') == 0:
              init_menu[eachDevice]['power_up_10_4'] = init_menu[eachDevice]['SUBMENU'].Append(wx.ID_ANY,'Power-up/Reset 10:4 Mode',"Gearbox Reset after Power On")
              init_menu[eachDevice]['power_up_10_10'] = init_menu[eachDevice]['SUBMENU'].Append(wx.ID_ANY,'Power-up/Reset 10:10 Mode',"Gearbox Reset after Power On")
              #gb2_gen25  = init_menu[eachDevice]['SUBMENU'].Append(wx.ID_ANY,'Set all 25G lanes to PRBS31'," ")
              #gb2_fifo25 = init_menu[eachDevice]['SUBMENU'].Append(wx.ID_ANY,'Set all 25G lanes to FIFO mode'," ")

              #gb2_gen10  = init_menu[eachDevice]['SUBMENU'].Append(wx.ID_ANY,'Set all 10G lanes to PRBS31'," ")
              #gb2_fifo10 = init_menu[eachDevice]['SUBMENU'].Append(wx.ID_ANY,'Set all 10G lanes to FIFO mode'," ")
              #gb2_ver  = init_menu[eachDevice]['SUBMENU'].Append(wx.ID_ANY,'Set all lanes to Auto Verify'," ")            
              
              self.menu_device = eachDevice
              self.Bind(wx.EVT_MENU, self.OnDeviceSpecific_GB2_10_4, init_menu[eachDevice]['power_up_10_4'])
              self.Bind(wx.EVT_MENU, self.OnDeviceSpecific_GB2_10_10, init_menu[eachDevice]['power_up_10_10'])
              #self.Bind(wx.EVT_MENU, self.OnDeviceSpecific_GB2_gen25,  gb2_gen25)
              #self.Bind(wx.EVT_MENU, self.OnDeviceSpecific_GB2_fifo25, gb2_fifo25)
              #self.Bind(wx.EVT_MENU, self.OnDeviceSpecific_GB2_ver25,  gb2_ver)
              #self.Bind(wx.EVT_MENU, self.OnDeviceSpecific_GB2_gen10,  gb2_gen10)
              #self.Bind(wx.EVT_MENU, self.OnDeviceSpecific_GB2_fifo10, gb2_fifo10)
            
        #----- CDR Menu Items -------------------------------------------------     
        for eachDevice in self.CDR_list:
          init_menu[eachDevice] = {}
          init_menu[eachDevice]['SUBMENU'] = wx.Menu()
          init_menu['WXMENU'].AppendMenu(wx.ID_ANY,eachDevice+' specific routines...',init_menu[eachDevice]['SUBMENU'])            
        ...
        for eachDevice in self.Device_list:
        ...
        #----------------------------------------------------------------------
        #Check for GearBox 2 in Device List
        #self.GB2_list = [] 
        #for eachDevice in ipvar.System.keys():
        if(len(self.GB2_list)):
          nextTab = 'Eye Scan'
        ...
        #----------------------------------------------------------------------
        #Add BER and Eyescan for CDR and GB1
        if(len(self.CDR_list+self.GB1_list)):
          ...
          nextTab = 'Eye Scan'
          self.eyeScanPanel = ipeyeDB.eyeScanPanel(self.notebook, -1)
          self.notebook.AddPage(self.eyeScanPanel,nextTab)               
        #----------------------------------------------------------------------        
        #Check for GearBox 1 in Device List , add CAUI Controls
        if len(self.GB1_list): 
          nextTab = 'CAUI Eye Scan'
          self.notebook.pages[nextTab] = MyNotebookPage()
          self.notebook.pages[nextTab].create(self.notebook,nextTab,self,blank=1)
          self.notebook.pages[nextTab].addCauiEyeScanControls(self,ipvar.CDR_Name,self.GB1_list)
    ...
    def FindCustomButtons(self):
      ...
      for eachFile in local_files:
        if eachFile.find('gui_button_') == 0:
          ...
          self.custom_buttons[buttonName] = wx.Button(self.bmp1, -1, label=buttonName,size=(60, 25),pos=(x,y))
          self.custom_buttons[buttonName].SetToolTipString('Run from '+ eachFile)
          self.custom_buttons[buttonName].run = eachFile
          self.custom_buttons[buttonName].name = buttonName
    ...
    def OnDeviceSpecific_GB2_10_4(self,evt):  
      startup_dictionary = {"target":"GB2", "28Gdata":"no", "mode":"10_4","quiet":"yes"}
      execfile("../scripts/startup_gb2.py",globals(),startup_dictionary)
      self.FullSystemRead2Write()
    def OnDeviceSpecific_GB2_10_10(self,evt):  
      startup_dictionary = {"target":"GB2", "28Gdata":"no", "mode":"10_10","quiet":"yes"}
      execfile("../scripts/startup_gb2.py",globals(),startup_dictionary)
      self.FullSystemRead2Write()
