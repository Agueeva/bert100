...
class Meter(LabDev):
  ...
  def get_v(self, chNum):
    if self.modelType == METER_34970:
      configure_mode = self.viDev.ask(":CONF? (@%s)"%self.chDict[chNum])
      channel_mode  = self.viDev.ask(":ROUT:SCAN?")
      if configure_mode.find("VOLT") == -1:
        self.viDev.write(":CONFigure:VOLTage:DC DEF,DEF,(@%s)"%self.chDict[chNum])  
        #print "voltage",self.chDict[chNum]
      if channel_mode.find(str(self.chDict[chNum])) == -1:
        self.viDev.write(":ROUT:SCAN (@%s)"%self.chDict[chNum])
        #print "\n\n\nchannel ",self.chDict[chNum]
      if channel_mode.find(',') != -1:
        self.viDev.write(":ROUT:SCAN (@%s)"%self.chDict[chNum])
      self.viDev.ask("*OPC?") 
      rsL = self.viDev.ask(":READ?")
  ...
  def get_freq(self, chNum):
    if self.modelType == METER_34970:
      #print "FREQ ",self.chDict[chNum]
      configure_mode = self.viDev.ask(":CONF? (@%s)"%self.chDict[chNum])
      channel_mode  = self.viDev.ask(":ROUT:SCAN?")
      if configure_mode.find("FREQ") == -1:
        self.viDev.write(":CONFigure:FREQuency DEF,DEF,(@%s)"%self.chDict[chNum])  
      if channel_mode.find(str(self.chDict[chNum])) == -1:
        self.viDev.write(":ROUT:SCAN (@%s)"%self.chDict[chNum])
      if channel_mode.find(',') != -1:
        self.viDev.write(":ROUT:SCAN (@%s)"%self.chDict[chNum])
  ...
  def get_res(self, chNum):
    if self.modelType == METER_34970:
      ##print "RES ",self.chDict[chNum] # fixme
      configure_mode = self.viDev.ask(":CONF? (@%s)"%self.chDict[chNum])
      channel_mode  = self.viDev.ask(":ROUT:SCAN?")
      if (configure_mode.find("RES")) == -1:
        self.viDev.write(":CONFigure:RESistance DEF,DEF,(@%s)"%self.chDict[chNum])
        self.viDev.ask("*OPC?")
      if channel_mode.find(str(self.chDict[chNum])) == -1:
        self.viDev.write(":ROUT:SCAN (@%s)"%self.chDict[chNum])
      if channel_mode.find(',') != -1:
