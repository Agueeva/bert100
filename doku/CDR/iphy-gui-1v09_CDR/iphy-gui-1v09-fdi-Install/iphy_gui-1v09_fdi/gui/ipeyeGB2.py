...
#11/27 - Lock CTLE prior to running EyeScan
import wx
import re
import time
...
class eyeScanPanel(wx.Panel):
  ...
  def __init__(self, parent, *args, **kwargs):
    ...
    self.startButton.controls = self 
    ...
    for eachDevice in ipvar.System.keys():
      if (re.search(r'\w+-\w+',eachDevice)):
        ...
        if(gb2) :
          #print 'ipeyeGB2: %s'%gb2.group(1)
          self.GB2_list.append(gb2.group(1))               
        

      
    print 'ipeyeGB2: %s'%self.GB2_list
    ...
    for i in range(self.lanes):
      choice_list.append('Rx%d'%i)

    self.m_checkBoxRx = wx.RadioBox(self.panel,-1,label='Receiver', pos=(rx,ry+44),choices=choice_list,majorDimension=5, style = wx.RA_SPECIFY_COLS)
    ...
    self.cdrSave = wx.CheckBox(self.panel,-1,label='Capture CDR PI',pos=(rx+5,ry+115))
    self.eyeColour = wx.CheckBox(self.panel,-1,label='ColourMap',pos=(rx+115,ry+115))
    self.eyeColour.SetValue(True)
    self.legacyCSV = wx.CheckBox(self.panel,-1,label='Legacy CSV',pos=(rx+195,ry+115))
  ...
  def DoVerticalStepsF(self,myRange,myX,forceFull=False,refreshWindow=True,debug=False):
    ...
    for vertical in myRange:
      ...
      #else:
        #self.panel.Refresh()
        #if(debug): print 'Live Height: %d'%vertical
        #return(self.maxVoffset)
  ...
  #------------------------------------------------------------------
  def YesNo(self,parent, question, caption = 'Yes or no?'):
  ...
  #------------------------------------------------------------------
  def Do(self,event,GB1_sim=False):     
    self.stop = 0
    ...
    #-----
    print '-'*70
    self.device_select = self.m_deviceBox.GetStringSelection()
    print "%s selected" %(self.device_select)
    
	  #Get Instance Name
    gb2 = re.search(r'(GB2\w*)-',self.device_select)
 
    self.device = gb2.group(1)             
    ...
    if(re.search('-Host',self.device_select)):
      base_offset = 0x2080 + self.rxn*0x100
      print 'Host Base Offset: 0x%04x(%d)'%(base_offset,base_offset)
      
    if(re.search('-Opt',self.device_select)):
      base_offset = 0x1080 + self.rxn*0x100
      print 'Optical Base Offset: 0x%04x(%d)'%(base_offset,base_offset)


  

    #self.Warn(self,'TEST Mode - Using GB1 base offset(0x180)')
    if(GB1_sim):
      self.testMode = self.Warn(self,'TEST Mode - Use GB1 base offset(0x180)')
      base_offset = 0x180 + self.rxn*0x100
      print 'TEST MODE - Base Offset: 0x%04x'% base_offset

      
    #dlg = wx.MessageDialog(self, 'Test Mode', 'Warning', wx.OK | wx.ICON_WARNING)
    #dlg.ShowModal()
    #dlg.Destroy()  
    #esEnable
    self.eqCTRL         = '30.' + str(base_offset+3)             
    self.esCTRL1        = '30.' + str(base_offset+7)             #391
    self.esCTRL2        = '30.' + str(base_offset+8)             #392
    self.esEnable       = '30.' + str(base_offset+7)  + '.0'    
    self.esSenseOffset  = '30.' + str(base_offset+7)  + '.15:8'
    self.esVertOffset   = '30.' + str(base_offset+8)  + '.15:8'
    self.esHorizDir     = '30.' + str(base_offset+8)  + '.7'
    self.esHorizOffset  = '30.' + str(base_offset+8)  + '.6:0'
    self.esResult       = '30.' + str(base_offset+9)  + '.15:0'
    self.saAdpOffset6   = '30.' + str(base_offset+53) + '.15:8'   # sense amp offset 6 (30.437)
    self.pi             = '30.' + str(base_offset+40) + '.9:0'    # PI code            (30.424)
    self.ctle_state     = '30.' + str(base_offset+37) + '.3:0'    # ctle               (30.421)
    #self.saAdpOffset6   = '30.' + str(437+(self.rxn*256)) + '.15:8'   # sense amp offset 6
    #self.pi             = '30.' + str(424+(self.rxn*256)) + '.9:0'   # PI code
    #self.ctle_state     = '30.' + str(421+(self.rxn*256)) + '.3:0'
	
	  #----- read CTLE State and Lock ------
    self.ctle_adapt = ipallreg.regRead(self.device+'::'+self.eqCTRL+'.0')  # read and save Adapt Enable
    if (self.ctle_adapt):
      print 'Lock CTLE'
      ipallreg.regWrite(self.device+'::'+self.eqCTRL+'.0',0) 	  
    ...
    #----- return CTLE to previous state ------
	
    self.ctle_lock = ipallreg.regRead(self.device+'::'+self.eqCTRL+'.0')  # read and save Adapt Enable
    if (self.ctle_adapt):
      print 'Enable CTLE Adapt'
      ipallreg.regWrite(self.device+'::'+self.eqCTRL+'.0',1) 
    ...
    fid.write(lastline)
    fid.write(lastline1)
    fid.write('SA Offset,%d'%self.saOffset)
