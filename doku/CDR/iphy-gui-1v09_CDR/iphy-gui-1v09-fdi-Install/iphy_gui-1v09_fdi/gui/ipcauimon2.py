#9/28 Fix 4pt to be non-destructive
#10/18 Remove redundant calls for speed up
#10/18 Support GB0M instance call - Add ',device' to memWrite/Read Calls
#10/18 All button only selects lanes that have CAUI input cdr lock 30.1546[9:0]
#11/01 get_caui_4ptF, faster non-destructive 4pt
#11/06 Add Locked button to only select lanes with cdr lock, change All back to select all
#11/20 - cauimonPanel take device list as a parameter
import wx
import re
import time
...
#--------------------------------------------------------------------
def get_caui_4ptF(cLane,device='GB0',debug=False):
  global memWrite, memRead
  if(debug): print "Clane = %d, x=%d, y=%d"%(cLane)
  if cLane < 3:
    common = 'common0'
  elif cLane < 7:
    common = 'common1'
  ...
  memWrite('0x25',cLane,0x48,device)
  memWrite('0xe',cLane,0x0,device)
  memWrite('0xf',cLane,0x0,device)

  #memWrite('0x25[3]',cLane,0,device)
  #print  "reg25: 0x%x"%memRead('0x25',cLane,device)
  #print  "reg36: 0x%x"%memRead('0x36',cLane,device)
  #print  "reg52: 0x%x"%memRead('0x52',cLane,device)
  #memWrite('0x36[6]',cLane,1,device)
  #memWrite('0x52[3]',cLane,0,device)   

  memWrite('0x25',cLane,0x40,device)      #0x25[3]=0
  memWrite('0x36',cLane,0x40,device)      #0x36[6]=1
  memWrite('0x52',cLane,0x54,device)      #0x52[3] = 0 
  if(debug):
    print  "reg25: 0x%x"%memRead('0x25',cLane,device)
    print  "reg36: 0x%x"%memRead('0x36',cLane,device)
    print  "reg52: 0x%x"%memRead('0x52',cLane,device)  
  ...
  #------  Scan X -----
  memWrite('0x1ac',common,0x01,device) # X
  memWrite('0x1ad',common,0x00,device) # Y
  memWrite('0x25',cLane,0x44,device) #START 0x25[2] = 1
  memWrite('0x25',cLane,0x40,device) #STOP 0x25[2] = 0
  horizontal=memRead('0x26',cLane,device)*2**6 + memRead('0x27[5:0]',cLane,device) 
  #------   Scan Y -----
  memWrite('0x1ac',common,0x00,device) # X [1:0]
  memWrite('0x1ad',common,0x10,device) # Y [7:4]
  memWrite('0x25',cLane,0x44,device) #START 0x25[2] = 1
  memWrite('0x25',cLane,0x40,device) #STOP 0x25[2] = 0
  vertical=memRead('0x26',cLane,device)*2**6 + memRead('0x27[5:0]',cLane,device)   
...
def get_caui_4pt(cLane,X=1,Y=1,device='GB0',debug=False):
  #global memWrite, memRead
  if(debug):
    print 'get_caui_4pt(Device: %s)'%device
  if cLane < 3:
    common = 'common0'
  elif cLane < 7:
    common = 'common1'
  ...
  # 0xd = 0x3
  # RXCALROAMYADJUST_LOCWREN = 0x1  
  # RXCALROAMXADJUST_LOCWREN = 0x1
  
  # RXCALROAMEYEMEASIN_LOCWREN(0x25[6]) = 0x1
  # RXCALROAMXADJUST(0xe) = 0x0
  # RXCALROAMYADJUST(0xf) = 0x0
  ...
  memWrite('0x52[3]',cLane,0,device) 
  memWrite('0x36[6]',cLane,1,device)
  memWrite('0x1ac[1:0]',common,X,device)
  memWrite('0x1ad[7:4]',common,Y,device)
  memWrite('0x1a8[7]',common,1,device)
  memWrite('0x25[2]',cLane,1,device)
  memWrite('0x25[2]',cLane,0,device)
  ...
  if debug:
    print  "RXCALEYEDIAGFSM_EYESUM_0x26[7:0]:",memRead('0x26',cLane,device)
    print  "RXCALEYEDIAGFSM_EYESUM_0x27[5:0]:",memRead('0x27[5:0]',cLane,device)
    print  "4-point eye value: ", memRead('0x26',cLane,device)*2**6 + memRead('0x27[5:0]',cLane,device)

  return (memRead('0x26',cLane,device)*2**6 + memRead('0x27[5:0]',cLane,device))


def get_caui_eye_xy(cLane,device='GB0',debug=False):
  global get_caui_4pt
  if(debug):
    print 'get_caui_eye_xy: %s'%device
  eye_4pt_x = get_caui_4pt(cLane,X=1,Y=0,device=device,debug=False) 
  eye_4pt_y = get_caui_4pt(cLane,X=0,Y=1,device=device,debug=False) 
...
class cauiMonitorPanel(wx.Panel):
  """  """
  def __init__(self, parent, *args, **kwargs):
    ...
    self.forceButton = wx.Button(self,-1,label="CFP",pos=(310,10),size=(75,24))
    self.forceButton.controls = self

    self.adaptButton = wx.Button(self,-1,label="Backplane",pos=(385,10),size=(75,24))
    self.adaptButton.controls = self

    self.statusButton = wx.Button(self,-1,label="EQ Status",pos=(385+75,10),size=(75,24))
    self.statusButton.controls = self

    self.eyeButton = wx.Button(self,-1,label="Refesh 4 point Eye",pos=(310,40),size=(120,24))
    ...
    for eachDevice in ipvar.System.keys():                       
      if (re.search(r'\w+-\w+',eachDevice)):
        ...
        if(gb1) :
          #print 'ipcauimon: %s'%gb1.group(1)
          self.GB1_list.append(gb1.group(1))
                
                
    self.device = self.GB1_list[0]    	
	
    self.m_deviceBox = wx.RadioBox(self, label="Device", pos=(rx+250, ry+20), choices=self.GB1_list,  majorDimension=1)
    ...
    for i in range (10):
      ...
      self.BERT[i].regs['prbsLock']      = self.device + '::30.' + str(20  + (i))
  ...
  def OnGo(self,event):
    ...
    for rxn in range(0,10):
      ...
      self.BERT[rxn].ber.SetBackgroundColour(wx.NullColor)
  ...
  def OnEyeRefresh(self,event,debug=False):
    self.device = self.m_deviceBox.GetStringSelection() 
    print "Refresh the 4 point Eye: %s"%self.device
    self.haltRead = 1
    for i in range(3):
    ...
    for i in range(10):
      if(self.BERT[i].checkbox.GetValue()):
        ...
        #(x_val,y_val) = get_caui_eye_xy(i,self.device,debug=False)
        # 11/1 - GJL Fast 4pt
        (x_val,y_val) = get_caui_4ptF(i,self.device,debug=False)
        (self.BERT[i].eye_x_val,self.BERT[i].eye_y_val) = (x_val,y_val)
  ...
  def select_all(self,event,debug=False):
    self.device = self.m_deviceBox.GetStringSelection()   
    if(debug):
      print 'Button All - cdr lock(0x%04x)'%cdr_lock
    for i in range (10):
      self.BERT[i].checkbox.SetValue(1)
  ...
  def select_locked(self,event,debug=True):
    ...
    if(debug):
      print 'Button Locked - cdr lock(0x%04x)'%cdr_lock
	
	#10/18 - Select only CAUI lanes with input cdr lock (30.1546[9:0])
    for i in range (10):
      if (cdr_lock & (1 <<i)):
        self.BERT[i].checkbox.SetValue(1)
      else:
