  ...
  def __init__(self, parent, interface,GB1_sim=False):
    ...
    if(interface == 'Host'):
      ...
      if(GB1_sim):
        ...
        caption = 'BER-Optical'
        question = 'TEST MODE - Use GB1 base offset (0x180)'
    ...
    self.button_all  = wx.Button(self,-1,'All',pos=(546,ry+82),size=(24,22))
    self.button_locked  = wx.Button(self,-1,'Locked',pos=(526,ry+82),size=(42,22))   
    self.button_locked.Show(False)
    ...
    for i in range (10):
      ...
      wx.StaticText(self.panel,-1,"CTLE",   pos=(self.rx+9,self.ry+63+(i/2)*self.i_y_offset)).SetFont(font)
      ...
      self.BERT[i].widgets['ctle'].name = 'ctle'      

      self.BERT[i].widgets['ppm'] = wx.StaticText(self,-1,'ppm',size=(50,22),pos=(self.rx+100,self.ry+63+(i/2)*self.i_y_offset),style=wx.RAISED_BORDER|wx.ALIGN_CENTER|wx.ST_NO_AUTORESIZE)
  ...
  def ReadTheWidgetRegisters(self,zeroThem=False,debug=False):
    ...
    for i in range (0,10):
      if(self.BERT[i].checkbox.GetValue()):
        ...
        #Read CTLE
        self.BERT[i].ctle  = ipallreg.regRead(self.device + '::30.' + str(self.base_offset  + (i*0x100)+37) +'.3:0')
    ...
    for rxn in range(0,10):
      if(self.BERT[rxn].checkbox.GetValue()):
        ...
        for eachWidget in self.BERT[rxn].widgets:
          ...
          elif self.BERT[rxn].widgets[eachWidget].name.find('ctle') == 0: 
            self.BERT[rxn].widgets[self.BERT[rxn].widgets[eachWidget].name].SetLabel(str('%d'%self.BERT[rxn].ctle))
