...
class eyeScanPanel(wx.Panel):
  ...
  def Do(self,event):     
    ...
    if(re.search('-Host',self.device)):
      base_offset = 0x2000 + self.rxn*0x100
      print 'Host Base Offset: 0x%04x(%d)'%(base_offset,base_offset)
      
    if(re.search('-Opt',self.device)):
      base_offset = 0x1000 + self.rxn*0x100
      print 'Opt Base Offset: 0x%04x(%d)'%(base_offset,base_offset)
