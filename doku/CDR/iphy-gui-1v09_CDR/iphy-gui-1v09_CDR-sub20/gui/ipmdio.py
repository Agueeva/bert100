...
def gb_read(devAddr,addr):
  #print "will read gb",devAddr,' ',addr
  if devAddr == 8: 
    regWrite('CFP:1.40962',0x0000 + addr)
  else:
    regWrite('CFP:1.40962',0x8000 + addr)	
  sleep(0.05)	
  regWrite('CFP:1.40960',0xcf00)
  sleep(0.05)
  regWrite('CFP:1.40961',0x6b11)
  sleep(0.1)
  #print addr,',',int(regRead('CFP:1.41010'))-32768,',',regRead('CFP:1.41011')

  return(regRead('CFP:1.41011'))
  
def gb_write(devAddr,addr,data,debug=False):
  if(debug): print "will write gb",devAddr,' ',addr,' 0x%04x' % data
  if devAddr == 8: 
    regWrite('CFP:1.40962',0x0000 + addr)
  else:
    regWrite('CFP:1.40962',0x8000 + addr)		
  sleep(0.05)
  regWrite('CFP:1.40963',data)
  sleep(0.05)
  regWrite('CFP:1.40960',0xcf00)
  sleep(0.05)
  regWrite('CFP:1.40961',0x6b00)
  sleep(0.05)
  #sleep(0.05)
  #print addr,',',int(regRead('CFP:1.41010'))-32768,',',regRead('CFP:1.41011')
...
#  returned = pointer(valC)
  if '-ATE_log' in sys.argv:
    ...
    fid.close()  
    fid1 = open('ate_log.py','a')
    fid1.write(("regWrite(device_name+\'::%s.%s\',0x%04x)\n") %(devAddr,regAddr,wVal))
    fid1.close()  
...
def regValRead(addrRef):
  ...
  if ipvar.MDIO_INIT:
    ...
    elif portAddr == -2:  # SEI catch the optics module and use the tunnel
      rVal = gb_read(devAddr,regAddr)	  
    else:
...
def regWrite(addrRef, wVal):
  ...
  if nBits<REG_LEN: # bitslice: read reg. first
    ...
    ipmdio_wr(portAddr, devAddr, regAddr, rVec.value) # WRITE
  else:
    #print 'regWrite(%s,%s) = portAddr(%d),nBits(%d),REG_LEN(%d)'%(addrRef,wVal,portAddr,nBits,REG_LEN)
    ipmdio_wr(portAddr, devAddr, regAddr, wVec.value) # WRITE
#-------------------------------------------------------------------------------
...
def ipmdio_wr(portAddr, devAddr, regAddr,wVal,debug=True):
  #print 'ipmdio_wr: %d.%d.%s'%(portAddr,devAddr,regAddr)
  #if ipvar.MDIO_INIT:
  if portAddr == -1:  # catch the optics module and use the tunnel
    cdrRef = int(re.match('M-CDR(\d)::.*',addrRef).group(1))
    cdr_write(cdrRef,regAddr,wVal)
