 ...
 */
void
MD5Result (uint8_t digest[16],	/* message digest */
     xy_md5ctx *context)		/* context */
{
	...
	MD5Loop (context, PADDING, padLen);

	/* Append length (before padding) */
	MD5Loop (context, bits, 8);
 ...
 */
void
MD5_EncodeString(uint8_t *digest,const char *string)
{
...
static int8_t
cmd_md5(Interp * interp, uint8_t argc, char *argv[])
{
	uint8_t digest[16];
	uint8_t i;
	if(argc > 1) {
		MD5_EncodeString(digest,argv[1]);
		for(i = 0; i < 16; i++) {
...
INTERP_CMD(md5Cmd,"md5", cmd_md5, "md5 string # Calculate md5sum of string");
/**
 *****************************************************************
