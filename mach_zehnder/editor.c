...
#define EDITOR_MAX_CMDLINE_LEN (50)
#define HISTORY_LINES (7)
#define PROMPT "MachZehnder > "
 ...
 ******************************************************************************
 */
static void editor_submit_line(Editor * ed)
{
	Line *curr, *prev;
 ...
 ***********************************************************************
 */
static void editor_insert(Editor * ed, uint8_t c)
{
	...
	//Editor_OutPStr(ANSI_insertchar);
	Editor_OutChar(c);
 ...
 **********************************************************************
 */
static inline void editor_backspace(Editor * ed)
{
 ...
 **********************************************************************
 */
static inline void editor_del(Editor * ed)
{
 ...
 **********************************************************************
 */
static inline void editor_goto_x(Editor * ed, uint8_t pos)
{
	while (ed->cursor > pos) {
		ed->cursor--;
		Editor_OutPStr(ANSI_left);
...
editor_redraw(Editor *ed)
{
        ...
        editor_goto_x(ed,0);
	/* Cursor left by 60 times */
	Editor_OutPStr("\033[60D");
 ...
 * Move one entry down in history. Done only if the
...
static void 
feed_editor(void *clientData, uint8_t c)
{
	Editor *ed = (Editor *) clientData;
	Line *line = &ed->lines[ed->current_line];
	//Con_Printf_P("Got char %02x\n",c);
	switch (ed->state) {
	case ED_STATE_CR:
	case ED_STATE_NL:
 ...
 ****************************************************************************************************
 */
void Editor_Init(Editor_LineSinkProc * LineSinkProc, void *LineSinkData)
{
	Editor *ed;
	ed = &g_editor;
	...
	ed->state = ED_STATE_IDLE;
	Con_RegisterSink(feed_editor,ed);
	//TWITerm_RegisterSink(feed_editor,ed);
	Editor_OutPStr( /* "\n" */ PROMPT);
