...
iShuffleRead(uint16_t uInp, uint8_t uStartBit, uint8_t uNumBit)
{
	...
	if (uStartBit + uNumBit > 16) {
		Con_Printf_P("CDR Bug, startbit + numbit > 16, %u + %u \n",uStartBit,uNumBit);
		return 0;
	}
...
//raise Exception("No CDR device detected at the specified PHYADDR")
	...
	if (uVal == 0x740) {
		Con_Printf_P("CDR device detected\n");
	} else {
		Con_Printf_P("No CDR device detected at addr %u\n",phy_addr);
		return -1;
	}
...
cmd_cdr(Interp * interp, uint8_t argc, char *argv[])
{
	...
	if ((argc == 3) && (strcmp(argv[1], "init") == 0)) {
		uint8_t cdr = astrtoi16(argv[2]);
		Con_Printf_P("Calling Olgas CDR_InitCdr for CDR %u\n", cdr);
		Cdr_InitCdr(cdr);
		return 0;
	} else if (argc == 3) {
